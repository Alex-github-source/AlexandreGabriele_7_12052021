<template>
  <div class="card mb-4 w-75 mx-auto">
    <div class="card-header d-flex justify-content-between">
      <div>
          Post by <em class="text-secondary">{{post.User.pseudo}}</em> <span> {{ post.createdAt | moment("from","now")}}</span>
      </div>
    
      <div v-if="this.isAdmin=== 'true'|| userId==post.User.id ">
         <modale :postId=post.id  v-bind:revele="revele" v-bind:toggleModale="toggleModale"></modale>
          <div
...
linkurl(){
    ...
    deletePost(n) {
             n=this.postId
            let confirmUserDeletion = confirm("voulez-vous vraiment supprimer ce post?");
            if (confirmUserDeletion == true) {
                axios.delete("http://localhost:3000/api/posts/" + n, {headers: { "Authorization": "Bearer " + localStorage.getItem("token") },})
                .then((res)=> {
                    console.log(res);
